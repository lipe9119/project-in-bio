    ...
    <div className="my-20 flex flex-col items-center gap-16">
      ...
      <div className="flex gap-3">
        <div className="flex flex-col gap-3">
          {faqItems.slice(0, Math.ceil(faqItems.length/2)).map(({ title, description }) => {
            return <FAQItem key={title} title={title} description={description} />;
          })}
        </div>

        <div className="flex flex-col gap-3">
          {faqItems.slice(Math.ceil(faqItems.length/2)).map(({ title, description }) => {
            return <FAQItem key={title} title={title} description={description} />;
          })}
        </div>
      </div>
...
function FAQItem({ title, description }: FQAItemProps) {
  return (
    <div className="w-[351px] h-min flex flex-col gap-3 p-5 rounded-2xl border border-border-primary bg-background-primary">
      ...
      <p className="border-x-content-body">{description}</p>
    </div>
  );
}
